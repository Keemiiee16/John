  ...
  try {
    ...
    if (i.isButton()) {
        ...
        try {
          ...
          await user.send({
            embeds:[new EmbedBuilder()
              ...
              .setDescription(`**${app.characters.name}** was approved for **${listing.position}** at **${listing.employer_name}**.`)
              .addFields(
      ...
      if (id === "admin_job_app_decline") {
        ...
        try {
          const user = await client.users.fetch(app.characters.owner_discord_id);
          await user.send(`❌ **${app.characters.name}**'s application for **${app.general_jobs?.position || "the job"}** at **${app.general_jobs?.employer_name || "the employer"}** was declined.`);
        } catch (e) {
      ...
      if (id === "bank_joint") {
        ...
        const existing = (memberships || []).map(m => {
        ...
        return respond(i,{
          embeds:[new EmbedBuilder()
            ...
            .setDescription(existing || "This character is not on any joint accounts yet.")],
          components:[buttonRow([
      ...
      if (id === "property_split_accept") {
        ...
        const member = unwrap(await db.from("property_split_payers")
          .select("id,discord_user_id,status,split_group_id")
        ...
        unwrap(await db.rpc("refresh_property_split_status",{p_split_group_id:member.split_group_id}),"refresh split");
        return respond(i,{content:"✅ You accepted your share of the housing payment."});
      }

      if (id === "property_split_decline") {
        const splitMemberId = arg;
        const member = unwrap(await db.from("property_split_payers")
          .select("id,discord_user_id,split_group_id")
          .eq("id",splitMemberId).limit(1),"split payer")?.[0];
        if (!member || member.discord_user_id !== i.user.id) throw new Error("This split-payment request is not for you.");
        unwrap(await db.from("property_split_payers").update({status:"declined"}).eq("id",splitMemberId),"decline split payment");
        unwrap(await db.from("property_split_groups").update({status:"declined"}).eq("id",member.split_group_id),"decline split group");
        return respond(i,{content:"❌ You declined the split-payment request."});
      }
      ...
      if (id === "property_contract_accept") {
        ...
        return respond(i,{
          content:`✅ Housing contract accepted.${accepted?.unit_number ? ` Your assigned unit is **${accepted.unit_number}**.` : ""}`
        });
      }
      ...
      if (id === "property_contract_decline") {
        ...
        return respond(i,{content:"❌ Housing contract declined. The unit/property is available again."});
      }
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "property_menu") {
        ...
        if (value === "mine") {
          const rows = unwrap(await db.from("property_residents")
            .select("property_id,unit_id,is_payer,properties(name,property_type,ownership_status,monthly_cost_voro),property_units(unit_number,status)")
      ...
      if (id === "job_menu") {
        ...
        if (value === "mine") {
          ...
          return respond(i,{
            embeds:[new EmbedBuilder()
              .setTitle(`💼 ${c.name}'s Jobs`)
              .setDescription(jobs?.length ? jobs.map(j =>
                `**${j.position}** — ${j.employer_name}\n${nova(j.pay_amount_voro)} • ${j.pay_type}${j.pay_schedule ? ` • ${j.pay_schedule}` : ""} • **${j.status}**`
              ).join("\n\n") : "No jobs yet.")]
          });
        }
      }
      ...
      if (id === "job_listing") {
        ...
        return respond(i,{
          ...
          components: existing?.length ? [] : [buttonRow([
            {id:`job_apply:${job.id}:${c.id}`,label:"Apply",style:ButtonStyle.Success}
          ])]
        });
      }
      ...
      if (id === "manage_item") {
        ...
        if (arg1 === "properties") {
          actionOptions.splice(1,0,{label:"Adjust Unit Count",value:"units"});
        }
    ...
    if (i.isModalSubmit()) {
      ...
      if (id === "joint_create_modal") {
        ...
        const member = await characterById(memberCharacterId);
        if (!member) throw new Error("Joint account member not found.");
        ...
        const result = unwrap(await db.rpc("create_joint_account",{
          p_name:name,
          p_account_type:type,
        ...
        const joint = Array.isArray(result) ? result[0] : result;

        try {
          const user = await client.users.fetch(member.owner_discord_id);
          await user.send(`🏦 **${owner.name}** added **${member.name}** to the Equity Financial joint account **${name}**.`);
        } catch {}

        return respond(i,{content:`✅ Joint ${type} **${name}** created for **${owner.name}** and **${member.name}**.`});
      }
      ...
      if (id === "vehicle_share_modal") {
        ...
        unwrap(await db.from("vehicle_access").upsert({
          vehicle_id:vehicleId,
          character_id:memberCharacterId,
          granted_by_character_id:owner.id,
          can_drive:canDrive,
          can_buy_gas:canGas,
          can_maintain:canMaintain,
          status:"active"
        },{onConflict:"vehicle_id,character_id"}),"share vehicle");

        try {
          const user = await client.users.fetch(member.owner_discord_id);
          await user.send(`🔑 **${owner.name}** shared **${vehicle.name}** with **${member.name}**. Permissions: ${[
            canDrive ? "Drive" : null, canGas ? "Buy Gas" : null, canMaintain ? "Maintain" : null
          ].filter(Boolean).join(", ")}.`);
        } catch {}

        return respond(i,{content:`✅ **${vehicle.name}** is now shared with **${member.name}**.`});
      }
      ...
      if (id === "create_general_job_modal") {
        ...
        if (!["salary","hourly","commission","custom"].includes(payType)) {
          throw new Error("Pay type must be salary, hourly, commission, or custom.");
        }
