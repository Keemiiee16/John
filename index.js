    ...
    if (i.isButton()) {
      ...
      if (id === "business_bulk_add") {
        ...
        const business = unwrap(await db.from("businesses")
          ...
          .eq("id",businessId)
          .limit(1),"bulk add business")?.[0];

        if (!business) throw new Error("Business not found.");

        return i.showModal(modal(
          `business_bulk_add_modal:${businessId}`,
          `Add Items — ${business.name}`,
          [
            {
              id:"items",
              label:"Up to 10 lines: Name | Price | Category",
              long:true,
              placeholder:"Black Hoodie | 75.00 | Clothing\\nCargo Pants | 120.00 | Clothing"
            }
          ]
        ));
      }
      ...
      if (id === "business_view_catalog") {
        ...
        return respond(i,{
          ...
          components:[buttonRow([
            {id:`business_bulk_add:${businessId}`,label:"Add More Items",style:ButtonStyle.Success}
          ])]
        });
      }
      ...
      if (id === "create_kind") {
        ...
        if (value === "property") {
          return i.showModal(modal("create_property_modal","Create Property",[
            ...
            {id:"rooms",label:"Bedrooms | Bathrooms",placeholder:"2 | 1.5"},
      ...
      if (id === "create_business_ownership") {
        ...
        if (value === "npc") {
          return i.showModal(modal("create_business_modal:npc:none","Create NPC Business",[
            ...
            {id:"items",label:"Items / Menu",long:true,required:false,placeholder:"Item | 19.99\\nItem | 25.00"}
          ]));
        }
      ...
      if (id === "manage_action") {
        ...
        if (value === "catalog" && kind === "businesses") {
          ...
          return respond(i,{
            ...
            components:[
              buttonRow([
                {id:`business_bulk_add:${itemId}`,label:"Add Up To 10 Items",style:ButtonStyle.Success},
    ...
    if (i.isModalSubmit()) {
      ...
      if (id === "business_bulk_add_modal") {
        ...
        const business = unwrap(await db.from("businesses")
          .select("id,name,business_type")
          .eq("id",businessId)
          .limit(1),"bulk add modal business")?.[0];
        ...
        if (lines.length > 10) throw new Error("You can add up to 10 items per batch.");

        const rows = [];
        const errors = [];

        for (let idx = 0; idx < lines.length; idx++) {
          const partsLine = lines[idx].split("|").map(x=>x.trim());
          const itemName = partsLine[0];
          const priceText = partsLine[1];
          const category = partsLine[2] || null;
          const priceVoro = toVoro(priceText);

          if (!itemName || priceVoro === null || priceVoro < 0) {
            errors.push(`Line ${idx+1}: use Name | Price | Category`);
            continue;
          }
        ...
        if (!rows.length) {
          throw new Error(errors.join(" • ") || "No valid items found.");
        }

        unwrap(await db.from("business_items").insert(rows),"bulk add business items");

        unwrap(await db.from("admin_audit_logs").insert({
          guild_id:i.guildId,
          moderator_discord_id:i.user.id,
          action_type:"business_bulk_add_items",
          details:{
            business_id:businessId,
            business_name:business.name,
            item_count:rows.length,
            items:rows.map(r=>({name:r.item_name,price_voro:r.price_voro,category:r.category}))
          }
        }),"audit bulk items");
        ...
        return respond(i,{
          content:`✅ Added **${rows.length} item${rows.length === 1 ? "" : "s"}** to **${business.name}**.${summary ? `\\n${summary}` : ""}${errors.length ? `\\n⚠️ Skipped: ${errors.join(" • ")}` : ""}`,
          components:[buttonRow([
            {id:`business_bulk_add:${businessId}`,label:"Add More Items",style:ButtonStyle.Success},
      ...
      if (id === "create_general_job_modal") {
        ...
        return respond(i,{
          content:`✅ General job created: **${position}** at **${employer}** — **${nova(payVoro)}** ${payType}${paySchedule ? ` • ${paySchedule}` : ""}.`
        });
      }
      ...
      if (id === "create_property_modal") {
        ...
        return respond(i,{content:`✅ Property **${row.name}** created at **${nova(cost)}/month**.`});
      }
      ...
      if (id === "create_business_modal") {
        ...
        const name = i.fields.getTextInputValue("name").trim();
        const type = i.fields.getTextInputValue("type").trim().toLowerCase();
        const rawItems = i.fields.getTextInputValue("items").trim();

        if (!["shop","restaurant"].includes(type)) throw new Error("Business type must be shop or restaurant.");
        ...
        if (rawItems) {
          const parsed = rawItems.split("\\n").map(line=>{
            const [itemName, priceText] = line.split("|").map(x=>x?.trim());
            const price = toVoro(priceText);
            if (!itemName || price === null) return null;
            return {
              business_id:business.id,
              item_name:itemName,
              price_voro:price,
              item_type:type === "restaurant" ? "consumable" : "owned",
              is_active:true
            };
          }).filter(Boolean);

          if (parsed.length) unwrap(await db.from("business_items").insert(parsed),"create business items");
        }
        ...
        return respond(i,{content:`✅ **${business.name}** was created as a ${type}.`});
      }
