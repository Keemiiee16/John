  ...
  try {
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "shop_gift_character") {
        ...
        let notified = false;
        try {
          const recipientUser = await client.users.fetch(recipient.owner_discord_id);
          await recipientUser.send({
            embeds:[new EmbedBuilder()
              .setTitle("🎁 You Received a Gift!")
              .setDescription(`**${buyer.name}** bought **${recipient.name}** **${r.item_name_out}**.\n\nCheck **/inventory** to see your gift.`)]
          });
          notified = true;
        } catch (e) {
          console.warn("[Shop gift recipient notification failed]",e.message);
        }

        return respond(i,{
          content:`🎁 **${buyer.name}** bought **${r.item_name_out}** for **${recipient.name}** for **${nova(r.price_voro_out)}**.${notified ? " The recipient was notified." : ""}`
        });
      }
      ...
      if (id === "inventory_give_character") {
        ...
        try{
          const recipientUser=await client.users.fetch(recipient.owner_discord_id);
          await recipientUser.send({embeds:[new EmbedBuilder().setTitle("🎁 You Received an Item!")
            .setDescription(`**${giver.name}** gave **${recipient.name}** **${r?.item_name_out||item.item_name}**.\n\nCheck **/inventory** to see it.`)]});
          notified=true;
        }catch(err){
          console.warn("[Inventory gift recipient notification failed]",err.message);
        }
        ...
        return respond(i,{
          embeds:[new EmbedBuilder().setTitle("🎁 Item Given")
            .setDescription(`**${giver.name}** gave **${r?.item_name_out||item.item_name}** to **${recipient.name}**.${notified?"\n\n🔔 The recipient was notified.":""}`)],
