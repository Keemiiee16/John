  ...
  try {
    ...
    if (i.isButton()) {
      ...
      if (id === "vantage_add") {
        ...
        const cart=getVantageCart(i.user.id,c.id);
        const existing=cart.items.find(x=>x.category===action.category && x.size===action.size);
        if(existing) existing.quantity+=1;
        else cart.items.push({
          category:action.category,size:action.size,name:action.itemName,
          price_voro:Number(action.price_voro),quantity:1
        });
      ...
      if (id === "vantage_buy_now") {
        ...
        return respond(i,{
          embeds:[new EmbedBuilder().setTitle("🧾 Vantage Checkout")
            .setDescription(`${vantageCartLines(cart)}\n\n**Total: ${nova(vantageCartTotal(cart))}**`)],
          components:[buttonRow([
            {id:`vantage_checkout:${c.id}`,label:"Confirm Purchase",style:ButtonStyle.Success},
            {id:`vantage_continue:${c.id}`,label:"Keep Shopping"}
          ])]
        });
      }
      ...
      if (id === "vantage_clear") {
        const c=await ensureOwnedCharacter(arg,i.user.id);
        vantageCarts.set(vantageCartKey(i.user.id,c.id),{items:[],createdAt:Date.now()});
        return respond(i,{
      ...
      if (id === "vantage_checkout") {
        ...
        if(Number(account.balance_voro)<total) throw new Error(`Not enough Nova in ${BANK_NAME} Checking.`);
