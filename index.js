  ...
  try {
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "vehicle_transfer_character") {
        ...
        const vehicle = unwrap(await db.from("vehicles").select("id,character_id,name").eq("id",vehicleId).limit(1),"vehicle transfer character")?.[0];
        ...
        const result = unwrap(await db.rpc("transfer_vehicle_ownership",{
          p_vehicle_id:vehicleId,
          p_old_owner_character_id:oldOwner.id,
          p_new_owner_character_id:newOwner.id,
          p_old_owner_discord_id:i.user.id
        }),"transfer vehicle ownership");

        return respond(i,{content:`✅ **${vehicle.name}** ownership transferred from **${oldOwner.name}** to **${newOwner.name}**.`});
      }
