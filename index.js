  ...
  try {
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "inventory_item") {
        ...
        const actionToken = makeShopToken({
        ...
        if (edible) {
          buttons.push({id:`inventory_eat:${actionToken}`,label:"Eat / Drink",style:ButtonStyle.Success});
        } else {
          buttons.push({id:`inventory_use:${actionToken}`,label:"Use",style:ButtonStyle.Primary});
        }
        buttons.push({id:`inventory_give_token:${actionToken}`,label:"Give Item"});
      ...
      if (id === "inventory_use") {
        // ACKNOWLEDGE FIRST — do not do DB work before deferUpdate.
        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
          catch (error) {
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY USE ACK SKIPPED] ${error.code}`);
              return;
            }
            throw error;
          }
        ...
        if (item.is_consumable) {
          ...
          if (remaining <= 1) {
            // quantity=0 violates character_inventory_quantity_check,
            // so remove the row entirely when this is the last one.
        ...
        const categoryReactions = {
          clothing:[
            `${c.name} used **${item.item_name}** as part of their outfit.`,
            `${c.name} put **${item.item_name}** to use.`
          ],
          accessories:[
            `${c.name} used **${item.item_name}**.`,
            `${c.name} added **${item.item_name}** to the look.`
          ],
          electronics:[
            `${c.name} used **${item.item_name}** for a bit. 🔌`,
            `${c.name} powered up **${item.item_name}** and put it to use.`
          ],
          beauty:[
            `${c.name} used **${item.item_name}**. ✨`,
            `${c.name} reached for **${item.item_name}** and used it.`
          ],
          toys_games:[
            `${c.name} spent some time using **${item.item_name}**. 🎮`,
            `${c.name} played with **${item.item_name}** for a while.`
          ],
          home:[
            `${c.name} used **${item.item_name}** around the house.`,
            `${c.name} put **${item.item_name}** to use at home.`
          ],
          school_office:[
            `${c.name} used **${item.item_name}** to get something done. 📝`,
            `${c.name} put **${item.item_name}** to work.`
          ],
          gifts:[
            `${c.name} used **${item.item_name}**.`,
            `${c.name} finally put **${item.item_name}** to use.`
          ],
          misc:[
            `${c.name} used **${item.item_name}**.`,
            `${c.name} put **${item.item_name}** to use.`
      ...
      if (id === "inventory_eat") {
        ...
        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
            ...
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY EAT ACK SKIPPED] ${error.code}`);
              return;
            }
            throw error;
          }
        ...
        if (!item) throw new Error("That food/drink item is no longer available.");
        ...
        if (remaining <= 1) {
          // quantity=0 violates character_inventory_quantity_check,
          // so delete the row when the last serving is consumed.
          unwrap(await db.from("character_inventory")
            .delete()
            .eq("id",item.id)
