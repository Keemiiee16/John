  ...
  try {
    ...
    if (i.isButton()) {
      ...
      if (id === "inventory_give") {
        ...
        return respond(i,{
          ...
          components:[userSelectRow(`inventory_give_user:${c.id}:${inventoryId}`,"Choose recipient")]
        });
      }
    ...
    if (i.isUserSelectMenu()) {
      ...
      if (i.customId.startsWith("inventory_give_user:")) {
        ...
        return respond(i,{
          ...
          components:[selectRow(`inventory_give_character:${ownerCharacterId}:${inventoryId}:${targetUserId}`,"Choose recipient character",chars.map(c=>({label:c.name,value:c.id})))]
        });
      }
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "inventory_give_character") {
        const giver = await ensureOwnedCharacter(arg1,i.user.id);
        const parts = i.customId.split(":");
        const inventoryId = parts[2];
        ...
        return respond(i,{content:`🎁 **${giver.name}** gave **${r.item_name_out}** to **${recipient.name}**.`});
      }
      ...
      if (id === "inventory_item") {
        ...
        buttons.push({id:`inventory_give:${c.id}:${item.id}`,label:"Give Item"});
