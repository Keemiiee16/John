...
client.on(Events.InteractionCreate, async i => {
  ...
  try {
    ...
    if (i.isButton()) {
      ...
      if (id === "inventory_give_token") {
        const action = getShopToken(arg);
        const c = await ensureOwnedCharacter(action.inventoryCharacterId,i.user.id);
        ...
        const giveToken = makeShopToken({
          giverCharacterId:c.id,
          inventoryId:action.inventoryId
        });

        return respond(i,{
          content:"Choose the Discord user receiving this item:",
          components:[userSelectRow(`inventory_give_user:${giveToken}`,"Choose recipient")]
        });
      }
      ...
      if (id === "inventory_give") {
        const parts = i.customId.split(":");
        const c = await ensureOwnedCharacter(parts[1],i.user.id);
        const inventoryId = parts[2];

        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
          catch (error) {
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY GIVE ACK SKIPPED] ${error.code}`);
              return;
            }
            throw error;
          }
    ...
    if (i.isUserSelectMenu()) {
      ...
      if (i.customId.startsWith("inventory_give_user:")) {
        const token = i.customId.split(":")[1];
        const action = getShopToken(token);
        await ensureOwnedCharacter(action.giverCharacterId,i.user.id);
        ...
        return respond(i,{
          content:"Which character should receive the item?",
          components:[selectRow(`inventory_give_character:${nextToken}`,"Choose recipient character",chars.map(c=>({
            label:c.name,
            value:c.id
          })))]
        });
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (id === "inventory_give_character") {
        const action = getShopToken(arg1);
        const giver = await ensureOwnedCharacter(action.giverCharacterId,i.user.id);
        const recipient = await characterById(value);

        if (!recipient || recipient.owner_discord_id !== action.targetUserId) {
          throw new Error("Recipient character mismatch.");
        }
        if (recipient.id === giver.id) {
          throw new Error("Choose a different character to receive the item.");
        }
        ...
        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
            ...
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY GIVE ACK SKIPPED] recipient ${error.code}`);
              return;
            }
            throw error;
          }
        ...
        const result = unwrap(await db.rpc("give_inventory_item",{
          p_inventory_id:action.inventoryId,
          p_giver_character_id:giver.id,
          p_recipient_character_id:recipient.id,
          p_discord_user_id:i.user.id
        }),"give inventory item");
        const r = Array.isArray(result)?result[0]:result;
        ...
        try {
          ...
          await recipientUser.send({
            embeds:[new EmbedBuilder()
              ...
              .setDescription(`**${giver.name}** gave **${recipient.name}** **${r.item_name_out}**.\n\nCheck **/inventory** to see it.`)]
          });
          notified = true;
        } catch (e) {
          console.warn("[Inventory gift recipient notification failed]",e.message);
        }

        return respond(i,{
          content:`🎁 **${giver.name}** gave **${r.item_name_out}** to **${recipient.name}**.${notified ? " The recipient was notified." : ""}`
        });
      }
      ...
      if (id === "inventory_item") {
        // ACK FIRST. Character/database lookups must happen after Discord is acknowledged.
        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
          catch (error) {
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY ITEM ACK SKIPPED] ${error.code}`);
              return;
            }
            throw error;
          }
        ...
        const giveToken = makeShopToken({
          inventoryCharacterId:c.id,
          inventoryId:item.id
        });
        const buttons = [];

        // Eat/Use IDs only carry the inventory UUID, so they survive bot restarts
        // and stay well under Discord's 100-character custom-id limit.
        if (edible) {
          buttons.push({id:`inventory_eat:${item.id}`,label:"Eat / Drink",style:ButtonStyle.Success});
        } else {
          buttons.push({id:`inventory_use:${item.id}`,label:"Use",style:ButtonStyle.Primary});
        }
        buttons.push({id:`inventory_give_token:${giveToken}`,label:"Give Item"});
      ...
      if (id === "inventory_use") {
        ...
        // Global FAST PRE-ACK handles inventory_* before routing.
        const c = await currentCharacter(i);
        ...
        if (!item) throw new Error("That inventory item is no longer available for your active character.");
        ...
        if (item.is_consumable) {
          ...
          if (remaining <= 1) {
            unwrap(await db.from("character_inventory")
              .delete()
              .eq("id",item.id)
              .eq("character_id",c.id),"consume last used inventory item");
            remaining = 0;
          } else {
            remaining -= 1;
            unwrap(await db.from("character_inventory")
              ...
              .eq("id",item.id)
              .eq("character_id",c.id),"consume used inventory item");
          }
        }

        const categoryReactions = {
          clothing:[`${c.name} used **${item.item_name}** as part of their outfit.`,`${c.name} put **${item.item_name}** to use.`],
          accessories:[`${c.name} used **${item.item_name}**.`,`${c.name} added **${item.item_name}** to the look.`],
          electronics:[`${c.name} used **${item.item_name}** for a bit. 🔌`,`${c.name} powered up **${item.item_name}** and put it to use.`],
          beauty:[`${c.name} used **${item.item_name}**. ✨`,`${c.name} reached for **${item.item_name}** and used it.`],
          toys_games:[`${c.name} spent some time using **${item.item_name}**. 🎮`,`${c.name} played with **${item.item_name}** for a while.`],
          home:[`${c.name} used **${item.item_name}** around the house.`,`${c.name} put **${item.item_name}** to use at home.`],
          school_office:[`${c.name} used **${item.item_name}** to get something done. 📝`,`${c.name} put **${item.item_name}** to work.`],
        ...
        const choices = categoryReactions[item.category] || categoryReactions.misc;
        ...
        return respond(i,{
          embeds:[new EmbedBuilder()
            .setTitle("🧰 Use Item")
            .setDescription(`${reaction}${wasConsumed ? `\n\nRemaining: **${remaining}**` : "\n\nThis item stays in inventory."}`)]
        });
      }
      ...
      if (id === "inventory_eat") {
        ...
        const item = unwrap(await db.from("character_inventory")
          .select("id,item_name,category,quantity,is_active")
          .eq("id",arg)
          .eq("character_id",c.id)
          .eq("is_active",true)
          .limit(1),"eat inventory item")?.[0];

        if (!item) throw new Error("That food/drink item is no longer available for your active character.");
        if (item.category !== "food_drink") throw new Error("That item is not food or drink.");
        ...
        if (remaining <= 1) {
          unwrap(await db.from("character_inventory")
            .delete()
            .eq("id",item.id)
            .eq("character_id",c.id),"eat last inventory item");
          remaining = 0;
        } else {
          remaining -= 1;
          unwrap(await db.from("character_inventory")
            .update({quantity:remaining,updated_at:new Date().toISOString()})
            .eq("id",item.id)
            .eq("character_id",c.id),"eat inventory item update");
        }

        const reactions = [
          `${c.name} finished **${item.item_name}**. 😋`,
          `${c.name} had **${item.item_name}** and it hit the spot.`,
          `${c.name} ate/drank **${item.item_name}**. One less in the bag.`,
          `${c.name} enjoyed **${item.item_name}**. 🍽️`,
          `${c.name} grabbed **${item.item_name}** and finished it.`
        ];
        const reaction = reactions[Math.floor(Math.random()*reactions.length)];

        console.log(`[INVENTORY EAT COMPLETE] item=${item.id} remaining=${remaining}`);

        return respond(i,{
          embeds:[new EmbedBuilder()
            .setTitle("🍽️ Eat / Drink")
      ...
      if (id === "inventory_filter") {
        if (!i.deferred && !i.replied) {
          try { await i.deferUpdate(); }
            ...
            if (error?.code === 10062 || error?.code === 40060) {
              console.warn(`[INVENTORY FILTER ACK SKIPPED] ${error.code}`);
              return;
            }
            throw error;
          }
