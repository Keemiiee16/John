...
async def init_db():
    async with aiosqlite.connect(DB) as db:
        ...
        CREATE TABLE IF NOT EXISTS characters (
            -- Characters are global across servers because they are tied to owner_user_id, not guild/server ID.
...
def apply_template(template: str, **values) -> str:
    """Replace placeholders like {character}, {item}, {amount}, etc."""
    if not template:
        return ""
...
async def send_john_welcome(guild):
    message = (
        "👋 **Thanks for adding John!**\n\n"
        "To set up the server/admin systems, start with:\n"
        "`!admin`\n\n"
        "For the regular user command list, use:\n"
        "`!help`"
    )
    ...
    # Otherwise, find the first text channel John can talk in
...
async def help(ctx):
    embed = discord.Embed(
        ...
        description="Full command list for John’s RP systems.",
...
`!setavatar <name>` — upload image with command
`!chars`
`!switchchar <name>`
`!whoami`
`!deletechar <name>`
...
Admin:
`!setmoney <character> <wallet> [bank]`
""",
        inline=False
    )
...
Admin:
`!addhouse <name> <price> <description>`
`!removehouse <name>`
""",
        inline=False
    )
...
""",
    ...
    embed.add_field(
        name="✨ Custom Replies",
        value="""
`!setreply <type> <message>`
`!replies`

Types:
`buy`, `gas`, `work`, `daily`, `job`
""",
        inline=False
    )
...
async def admin(ctx):
    ...
    embed = discord.Embed(
        title="🛠️ John Admin Setup Menu",
        description="Use these commands to set up John for your RP server.",
        color=discord.Color.blue()
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!removeitem <item>`\n\n"
            "Example:\n"
            "`!additem apple 5 Fresh red apple`"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="💼 Job Setup",
        value=(
            "`!addjob <name> <min_pay> <max_pay>`\n"
            "`!setjobpay <name> <min_pay> <max_pay>`\n"
            "`!removejob <name>`\n"
            "`!setworkcd <seconds>`\n"
            "`!workcd`\n\n"
            "Example:\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!setfuelusage <amount>`\n\n"
            "Example:\n"
            "`!addcar civic 4500 Reliable starter car`"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!removehouse <name>`\n\n"
            "Example:\n"
            "`!addhouse apartment 1200 Cozy starter apartment`"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="✨ Custom Reply Setup",
        value=(
            "`!setreply <buy/gas/work/daily/job> <message>`\n"
            "`!replies`\n\n"
            "Example:\n"
            "`!setreply buy Thanks for shopping with us, {character}!`"
        ),
        inline=False
    )

    embed.add_field(
        name="👤 Character Reminder",
        value=(
            "Users create characters with:\n"
            "`!createchar <name> <trigger>`\n\n"
            "Example:\n"
            "`!createchar Emely E`\n"
            "Then they can speak with `E: hello`."
        ),
        inline=False
    )

    embed.set_footer(text="John Admin Menu • Use !help for regular user commands")
    await ctx.send(embed=embed)



@bot.command()
async def globalchars(ctx):
    await ctx.send(
        "🌐 **Global Characters:** Characters are saved to your Discord user ID, not one server. "
        "If John is added to another server while using the same hosted bot/database, your characters can still be used there with `!chars`, `!switchchar <name>`, and their trigger like `E: hello`."
    )
...
async def setavatar(ctx, name: str):
    ...
    if not ctx.message.attachments:
        return await ctx.send("Attach a PNG/JPG image with the command. Example: upload the image and type `!setavatar Emely`.")
    ...
    if not filename.endswith(allowed_extensions):
        return await ctx.send("Please upload an image file: PNG, JPG, JPEG, WEBP, or GIF.")
...
@bot.command()
async def setworkcd(ctx, seconds: int):
    if not ctx.author.guild_permissions.administrator:
        return await ctx.send("Admins only.")
    ...
    await set_setting("work_cooldown", str(seconds))

    minutes = seconds // 60
    remaining_seconds = seconds % 60

    if seconds == 0:
        await ctx.send("Work cooldown has been turned off.")
    elif remaining_seconds == 0:
        await ctx.send(f"Work cooldown set to **{minutes} minutes**.")
    else:
        await ctx.send(f"Work cooldown set to **{seconds} seconds**.")


@bot.command()
async def workcd(ctx):
    seconds = int(await get_setting("work_cooldown", "1800"))

    minutes = seconds // 60
    remaining_seconds = seconds % 60

    if seconds == 0:
        await ctx.send("Current work cooldown: **off**.")
    elif remaining_seconds == 0:
        await ctx.send(f"Current work cooldown: **{minutes} minutes**.")
    else:
        await ctx.send(f"Current work cooldown: **{seconds} seconds**.")



@bot.command()
async def setmoney(ctx, character_name: str, wallet: int, bank: int = None):
    if not ctx.author.guild_permissions.administrator:
        return await ctx.send("Admins only.")

    if wallet < 0:
        return await ctx.send("Wallet cannot be negative.")

    if bank is not None and bank < 0:
        return await ctx.send("Bank cannot be negative.")
    ...
    async with aiosqlite.connect(DB) as db:
        if bank is None:
            await db.execute(
                "UPDATE characters SET wallet = ? WHERE id = ?",
                (wallet, character_id),
            )
            await db.commit()
            return await ctx.send(f"Set **{name}**'s wallet to **${wallet}**.")

        await db.execute(
            "UPDATE characters SET wallet = ?, bank = ? WHERE id = ?",
            (wallet, bank, character_id),
        )
        await db.commit()

    await ctx.send(f"Set **{name}**'s wallet to **${wallet}** and bank to **${bank}**.")
...
async def buycar(ctx, car_name: str):
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        if already_owned:
            return await ctx.send(f"**{name}** already owns **{car_name}**.")
...
async def buyhouse(ctx, house_name: str):
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        cur = await db.execute(
            "SELECT house_name FROM owned_houses WHERE character_id = ? AND house_name = ?",
            (char_id, house_name),
        )
        already_owned = await cur.fetchone()

        if already_owned:
            return await ctx.send(f"**{name}** already owns **{house_name}**.")
